...
export default function Info() {
  ...
  let lastRemainToken = ethers.utils.formatUnits('0', 18);
  let lastTokenSold = ethers.utils.formatUnits('0', 18);
      ...
      <Row justify="space-around">
        ...
        <Col md={{ span: 10, offset: 1 }} lg={{ span: 7, offset: 0 }}>
          <CardStyled>
            ...
            <Text>{new Intl.NumberFormat().format(Number(lastRemainToken) + Number(remainToken))} BOMB</Text>
          </CardStyled>
        </Col>
        <Col md={12} lg={7}>
          <CardStyled>
            <SubTitle>Token Sold</SubTitle>
            <Text>{new Intl.NumberFormat().format(Number(tokenSold) + Number(lastTokenSold))} BOMB</Text>
          </CardStyled>
        </Col>
      </Row>
    </Container>
